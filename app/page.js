...
const LEAGUES = [
  ...
  'Scottish Premiership', 'MLS', 'Liga MX', 'Brasileirao', 'Saudi Pro League',
  'Super Lig', 'A-League', 'Other / International'
]
...
const CLUB_LEAGUE_MAP = {
  ...
  'Real Madrid': 'La Liga', 'Barcelona': 'La Liga', 'Atletico Madrid': 'La Liga', 'Athletic Club': 'La Liga',
...
export default function Home() {
  ...
  const loadFranchises = async (userId) => {
    const result = await supabase.from('franchises').select('*').eq('user_id', userId).order('created_at', { ascending: false })
    if (!result.error) { setFranchises(result.data) }
  }

  const searchClubs = async () => {
    const result = await supabase.from('player_reference').select('active_club').ilike('active_club', '%' + clubName + '%').limit(50)
    if (!result.error) {
      const uniqueClubs = []
      const seen = {}
      for (let i = 0; i < result.data.length; i++) {
        const club = result.data[i].active_club
        if (club && !seen[club]) { seen[club] = true; uniqueClubs.push(club) }
      }
  ...
  const searchCfbTeams = () => {
    ...
    const matches = Object.keys(CFB_TEAM_CONFERENCE_MAP).filter(function(team) { return team.toLowerCase().indexOf(lower) !== -1 })
    setClubResults(matches.slice(0, 6))
    setShowClubResults(true)
  }

  const applyLeagueForClub = (club) => {
    if (selectedGame === 'EA CFB 27' && CFB_TEAM_CONFERENCE_MAP[club]) { setLeague(CFB_TEAM_CONFERENCE_MAP[club]) }
    else if (selectedGame === 'EA FC 26' && CLUB_LEAGUE_MAP[club]) { setLeague(CLUB_LEAGUE_MAP[club]) }
  }
  ...
  const handleCreateFranchise = async () => {
    setCreating(true)
    const franchiseResult = await supabase.from('franchises').insert({
      user_id: user.id, club_name: clubName, league: league, game: selectedGame, current_season: 1
    }).select().single()
  ...
  const handleDeleteFranchise = async (franchiseId) => {
    ...
    const result = await supabase.from('franchises').delete().eq('id', franchiseId)
    setDeletingId(null)
    setConfirmDeleteId(null)
    if (!result.error) { await loadFranchises(user.id) } else { alert(result.error.message) }
  }
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex justify-between items-start mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl text-neutral-100" style={{ fontFamily: "'Arial Black', 'Arial Bold', Impact, Haettenschweiler, sans-serif", fontWeight: 900, fontStyle: 'italic', letterSpacing: '-0.5px', transform: 'skewX(-6deg)', display: 'inline-block' }}>
            ...
            {!selectedGame ? (
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {GAMES.map(function(game) {
                  ...
                  return (
                    <button key={game.id} type="button" onClick={() => handleSelectGame(game)} disabled={!isLive}
                      className={'text-left rounded-lg p-4 border transition-colors ' + (isLive ? 'bg-neutral-800/50 border-neutral-800 hover:border-emerald-600 cursor-pointer' : 'bg-neutral-800/20 border-neutral-800 cursor-not-allowed opacity-60')}>
                      <p className="font-semibold text-neutral-100 text-sm">{game.label}</p>
                      <p className="text-neutral-500 text-xs mt-1">{game.sub}</p>
                      {isLive ? (<p className="text-emerald-400 text-xs mt-3 font-medium">Available</p>) : (<p className="text-neutral-500 text-xs mt-3 font-medium">Site in progress</p>)}
                    </button>
                  )
                })}
              </div>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div className="relative">
                    <label className="block text-xs font-medium text-neutral-400 mb-1">{selectedGame === 'EA CFB 27' ? 'Team Name' : 'Club Name'}</label>
                    <input type="text" placeholder={selectedGame === 'EA CFB 27' ? 'Start typing, e.g. Ohio...' : 'Start typing, e.g. Qu...'} value={clubName}
                      onChange={(e) => setClubName(e.target.value)} onFocus={() => clubResults.length > 0 && setShowClubResults(true)} onBlur={handleClubBlur}
                      className="w-full bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" autoFocus />
                    {showClubResults && clubResults.length > 0 && (
                      <div className="absolute z-20 mt-1 w-full bg-neutral-800 border border-neutral-700 rounded-lg shadow-lg overflow-hidden">
                        {clubResults.map((club, idx) => (
                          <button type="button" key={idx} onClick={() => handleSelectClub(club)} className="w-full text-left px-3 py-2 hover:bg-neutral-700 text-sm">{club}</button>
                        ))}
                      </div>
                    )}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-neutral-400 mb-1">{selectedGame === 'EA CFB 27' ? 'Conference' : 'League'}</label>
                    <select value={league} onChange={(e) => setLeague(e.target.value)} className="w-full bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
                      <option value="">{selectedGame === 'EA CFB 27' ? 'Select a conference...' : 'Select a league...'}</option>
                      {(selectedGame === 'EA CFB 27' ? CFB_CONFERENCES : LEAGUES).map((l) => (<option key={l} value={l}>{l}</option>))}
                    </select>
                  </div>
                </div>
                ...
                <div className="flex gap-2 justify-between">
                  <button type="button" onClick={() => setSelectedGame(null)} className="text-neutral-400 hover:text-neutral-200 text-sm px-3 py-2">Change Game</button>
                  <div className="flex gap-2">
