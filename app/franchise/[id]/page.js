...
/**
 * GENERALIZED ROSTER IMPORT
 * Add a new "case" here any time a new game's player database goes live.
 * This function is called automatically on franchise creation for every game,
 * so no other part of the app needs to be touched when adding a new game's import logic.
 */
async function importRosterForGame(supabase, game, franchiseId, teamName) {
  ...
  // Future games: add another `if (game === '...') { ... }` block here.
  return 0
}
...
export default function Home() {
  ...
  const handleCreateFranchise = async () => {
    ...
    // This one line now handles roster auto-import for ANY game.
    // Adding a new game later just means adding a case inside importRosterForGame above.
