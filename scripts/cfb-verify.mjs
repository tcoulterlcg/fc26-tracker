...
const nilNull = await sb.from('cfb_player_reference').select('id', { count: 'exact', head: true }).is('nil_value', null)
const nilTop = await sb.from('cfb_player_reference').select('player_name, team, overall_rating, nil_value').order('nil_value', { ascending: false }).limit(3)
console.log('rows missing nil_value:', nilNull.count, ' top NIL:', JSON.stringify(nilTop.data))
